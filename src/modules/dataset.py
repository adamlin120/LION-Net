...
def padding(seqs, max_seq_len=-1):
    ...
    if max_seq_len == -1:
        ...
        max_len = max_seq_len
    seqs = [seq[:max_len] + [0 for _ in range(max_len - len(seq))] for seq in seqs]
    return seqs
...
def gen_collate_fn(config, vocab, mode="train"):
    ...
    schema_vocab = pickle.load(open(save_dir / f"{mode}_schema_vocab.pkl", "rb"))
    ...
    def collate_fn(batch):
        n_data = len(batch)
        batch = {key: [data[key] for data in batch] for key in batch[0].keys()}
        output = {"n_data": n_data}
        context = [utter[0] for utter in batch["utterances"]]
        context = [[idx if idx < vocab_size else UNK for idx in cxt] for cxt in context]
        output["context"] = [torch.LongTensor(cxt) for cxt in context]
        output["context"] = [context.view(1, -1) for context in output["context"]]
        output["ext_context"] = [
        ...
        for idx, (service, intent_list) in enumerate(
            zip(batch["service_idx"], batch["intent_list"])
        ):
            ...
            output["active_intent"]["intent_idx"].append(torch.LongTensor(intent_idx))
            output["active_intent"]["service_idx"].append(torch.LongTensor(service_idx))
            if config.model_param.sch.type == "rnn":
            ...
            if mode != "test":
                output["active_intent"]["label"].append(torch.LongTensor([label]))
        # requested slots
        ...
        for idx, (service, slot_list) in enumerate(
            zip(batch["service_idx"], batch["slot_list"])
        ):
            slot_idx = slot_list
            ...
            if config.model_param.sch.type == "rnn":
                ...
                service_desc = [service2desc[idx] for idx in service_idx]
                service_desc = padding(service_desc, max_desc_len)
            output["requested_slots"]["slot_idx"].append(torch.LongTensor(slot_idx))
            ...
            if mode != "test":
                output["requested_slots"]["label"].append(torch.FloatTensor(label))
        # slot filling
        output["slot_filling"] = {}
        ...
        if mode == "test":
            output["slot_filling"]["possible_values"] = batch["possible_values"]
        ...
        for index, (service, slot_list, is_cat) in enumerate(
            zip(batch["service_idx"], batch["slot_list"], batch["is_categorical"])
        ):
            if mode != "test":
            ...
            if full_slot:
                    ...
                    for slot, values in slot_values:
                        ...
                        if multi_value:
                            ...
                            for value in values:
                                val += value + [EOS]
                            value_idx[pos] = [
                                idx if idx < vocab_size else UNK for idx in val
                            ]
                            value_ext_idx[pos] = val
                        else:
                            value_idx[pos] = [
                                idx if idx < vocab_size else UNK for idx in values[0]
                            ]
                ...
                if config.model_param.sch.type == "rnn":
                    context_slot_desc = [slot2desc[idx] for idx in context_slot_idx]
                    context_service_desc = [
                        service2desc[idx] for idx in context_service_idx
                    ]
                    value_slot_desc = [slot2desc[idx] for idx in value_slot_idx]
                    value_service_desc = [slot2desc[idx] for idx in value_service_idx]
                    context_slot_desc = padding(context_slot_desc, max_desc_len)
                    context_service_desc = padding(context_service_desc, max_desc_len)
                    value_slot_desc = padding(value_slot_desc, max_desc_len)
                    value_service_desc = padding(value_service_desc, max_desc_len)
                ...
                is_categorical = torch.LongTensor(cat_flag)
                value_idx = torch.LongTensor(padding(value_idx, max_val_len))
                value_ext_idx = torch.LongTensor(padding(value_ext_idx, max_val_len))
                value_mask = value_ext_idx == PAD
                if config.model_param.sch.type == "rnn":
                    context_slot_desc = torch.LongTensor(context_slot_desc)
                    context_service_desc = torch.LongTensor(context_service_desc)
                    value_slot_desc = torch.LongTensor(value_slot_desc)
                    value_service_desc = torch.LongTensor(value_service_desc)

            output["slot_filling"]["context_slot_idx"].append(context_slot_idx)
            output["slot_filling"]["context_service_idx"].append(context_service_idx)
            output["slot_filling"]["value_slot_idx"].append(value_slot_idx)
            output["slot_filling"]["value_service_idx"].append(value_service_idx)
            output["slot_filling"]["is_categorical"].append(is_categorical)
            if mode != "test":
                output["slot_filling"]["context_label"].append(context_label)
                if mode == "train":
                    output["slot_filling"]["value_idx"].append(value_idx)
                else:
                    output["slot_filling"]["value_idx"].append(None)
                output["slot_filling"]["value_ext_idx"].append(value_ext_idx)
                output["slot_filling"]["value_mask"].append(value_mask)
            else:
                output["slot_filling"]["value_idx"] = [
                    None for _ in range(output["n_data"])
                ]
            if config.model_param.sch.type == "rnn":
                output["slot_filling"]["context_slot_desc"].append(context_slot_desc)
                output["slot_filling"]["context_service_desc"].append(
                    context_service_desc
                )
                output["slot_filling"]["value_slot_desc"].append(value_slot_desc)
                output["slot_filling"]["value_service_desc"].append(value_service_desc)
...
def create_data_loader(filename, config, vocab, mode="train"):
...
def transfer(batch, device):
    batch["context"] = [context.to(device=device) for context in batch["context"]]
    batch["ext_context"] = [
        context.to(device=device) for context in batch["ext_context"]
    ]
    ...
    if "label" in batch["active_intent"]:
        batch["active_intent"]["label"] = [
            label.to(device=device) for label in batch["active_intent"]["label"]
        ]
    ...
    batch["requested_slots"]["slot_idx"] = [
        slot_idx.to(device=device) for slot_idx in batch["requested_slots"]["slot_idx"]
    ]
    ...
    if "label" in batch["requested_slots"]:
        batch["requested_slots"]["label"] = [
            label.to(device=device) for label in batch["requested_slots"]["label"]
        ]
    ...
    if "context_label" in batch["slot_filling"]:
        batch["slot_filling"]["context_label"] = [
            label.to(device=device) for label in batch["slot_filling"]["context_label"]
        ]
    ...
    batch["slot_filling"]["value_idx"] = [
        value_idx.to(device=device) if value_idx is not None else None
    ...
    if "value_mask" in batch["slot_filling"]:
        batch["slot_filling"]["value_mask"] = [
            mask.to(device=device) for mask in batch["slot_filling"]["value_mask"]
        ]
    ...
    if "context_service_desc" in batch["slot_filling"]:
        batch["slot_filling"]["context_service_desc"] = [
            service_desc.to(device=device)
            for service_desc in batch["slot_filling"]["context_service_desc"]
        ]
    ...
    if "value_service_desc" in batch["slot_filling"]:
        batch["slot_filling"]["value_service_desc"] = [
            service_desc.to(device=device)
            for service_desc in batch["slot_filling"]["value_service_desc"]
        ]
    return batch
