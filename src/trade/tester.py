...
class Tester:
    def __init__(self, config, device):
        ...
        nlp = spacy.load("en")
        self.tokenizer = spacy.lang.en.English().Defaults().create_tokenizer(nlp)
                ...
                for t, turn in enumerate(dialogue["turns"]):
                    if turn["speaker"] == "USER":
                        for f, frame in enumerate(turn["frames"]):
                            state = {}
                            state["active_intent"] = preds["act_preds"][count]
                            state["requested_slots"] = preds["req_preds"][count]
                            state["slot_values"] = preds["slot_value_preds"][count]
                            count += 1
    ...
    def run_epoch(self, epoch, mode):
        ...
        for b, d in enumerate(ebar):
            ...
            act_preds, req_preds, slot_value_preds = self.get_prediction(d, output)
    ...
    def get_prediction(self, batch, output):
        ...
        for i, o in enumerate(req_o):
            ...
            for j, val in enumerate(o):
                if val >= 0.5:
                    pred.append(
                        self.idx2slot[batch["requested_slots"]["slot_idx"][i][j]][1]
                    )
    ...
    def get_most_likely(self, emb, candidates, metric="cos"):
        if metric == "l2":
            ...
            likelihood = [
                np.dot(emb, b) / (LA.norm(emb) * LA.norm(b)) for b in candidates
            ]
